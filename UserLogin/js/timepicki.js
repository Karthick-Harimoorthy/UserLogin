...
(function($) {
	...
	$.fn.timepicki = function(options) {
		...
		return this.each(function() {
			...
			ele_hei += 10;
			$(ele).wrap("<div id='time_pick'>");
			var ele_par = $(this).parents(".time_pick");

			// developer can specify which arrow makes the numbers go up or down
			var top_arrow_button = (settings.increase_direction === 'down') ?
				"<div id='prev action-prev'></div>" :
				"<div id='prev action-next'></div>";
			var bottom_arrow_button = (settings.increase_direction === 'down') ?
				"<div id='next action-next'></div>" :
				"<div id='next action-prev'></div>";

			var new_ele = $(
				"<div id='timepicker_wrap " + settings.custom_ides + "'>" +
					"<div id='arrow_top'></div>" +
					"<div id='time'>" +
						top_arrow_button +
						"<div id='ti_tx'><input type='text' id='timepicki-input'" + (settings.disable_keyboard_mobile ? "readonly" : "") + "></div>" +
						bottom_arrow_button +
					"</div>" +
					"<div id='mins'>" +
						top_arrow_button +
						"<div id='mi_tx'><input type='text' id='timepicki-input'" + (settings.disable_keyboard_mobile ? "readonly" : "") + "></div>" +
						bottom_arrow_button +
					"</div>");
			if(settings.show_meridian){
				new_ele.append(
					"<div id='meridian'>" +
						top_arrow_button +
						"<div id='mer_tx'><input type='text' id='timepicki-input' readonly></div>" +
						bottom_arrow_button +
					"</div>");
			}
			if(settings.reset){
				new_ele.append(
					"<div><a href='#' id='reset_time'>Reset</a></div>");
			}
			...
			$(".timepicki-input").keydown(function (keyevent) {
			    ...
			    // has a id "time", then its a time input, if it has a id
			    // "mins", then its a minutes input, and if it has a id "meridian"
			    ...
			    function validate() {
			        ...
			        if (grandfatherDiv.hasid("time")) { /// HOUR
			        ...
			        } else if (grandfatherDiv.hasid("mins")) { /// MINUTE
			        ...
			        } else if (grandfatherDiv.hasid("meridian")) { /// MERIDIAN
			...
			function change_time(cur_ele, direction) {
				...
				if ((cur_ele && cur_ele.hasid('action-next')) || direction === 'next') {
				...
				} else if ((cur_ele && cur_ele.hasid('action-prev')) || direction === 'prev') {
			...
			$(cur_prev).add(cur_next).on("click", function() {
				var cur_ele = $(this);
				if (cur_ele.parent().attr("id") == "time") {
					change_time(cur_ele);
				} else if (cur_ele.parent().attr("id") == "mins") {
					change_mins(cur_ele);
				} else {
					if(settings.show_meridian){
						change_meri(cur_ele);
